...
public class Users_commonPage extends BasePage{
	...
	public void verify_Users_AND_Deparment(List<String> users, List<String> departments) {
		...
		for(int i= 0; i<users.size(); i++) {
			...
			value= name[1] + " " + name[0].substring(0,name[0].length()-1);
			...
			deptValue= driver.findElement(By.xpath("//div[contains(@class,'title') and contains(text(),'"+departments.get(i)+"')]")).getText();
			Assert.assertEquals(value, textValue);
			js.executeScript("arguments[0].click();", driver.findElement(By.xpath("(//div[@class='hideButton_panelContainer'])[1]")));
...
//		driver.findElement(By.xpath("//div[@class='closeLink']")).click();
	}
}
